...
class CardSet:
    ...
    def is_group(self):
        ...
        if len(self.cards) < 3 or len(self.cards) > NUM_OF_COLOURS:
            return False
        
        # Get the colour cards (regular cards) from a pack of cards
        colour_cards = card.Card.get_colour_cards(self.cards)   
    ...
    def is_run(self):
        ...
        if len(self.cards) < 3 or len(self.cards) > NUM_OF_CARDS_EACH_COLOUR:
            return False
        
        # Get the colour cards (regular cards) from a pack of cards
        colour_cards = card.Card.get_colour_cards(self.cards)
        # Get the count of Joker cards from a pack of cards
        joker_cards_count = len(card.Card.get_joker_cards(self.cards)) #len([card for card in cards if card.number == 30]) 
    ...
    # None -> bool
    def is_valid(self):
        return self.is_group() or self.is_run()              
