def handcards_play_rule(handcards):
    ...
    yellow_hand_first_unused_list = find_unused_element(first_hand_yellow_check_list, first_hand_yellow_final_even_list,
                                                        first_hand_yellow_final_odd_list)
    # print(yellow_first_unused_list)
    green_hand_first_unused_list = find_unused_element(first_hand_green_check_list, first_hand_green_final_even_list,
    ...
    green_hand_second_unused_list = find_unused_element(second_hand_green_check_list, second_hand_green_final_even_list,
                                                        second_hand_green_final_odd_list)
    blue_hand_second_unused_list = find_unused_element(second_hand_blue_check_list, second_hand_blue_final_even_list,
                                                       second_hand_blue_final_odd_list)
    pink_hand_second_unused_list = find_unused_element(second_hand_pink_check_list, second_hand_pink_final_even_list,
...
def cs_regular_play(verify_of_break_ice, handcards, boardcards):
    if verify_of_break_ice is True:
        ...
        handcards_used_list = handcards_play_rule(handcards)[0]  # 手牌单独组队后形成的新组
        handcards_used_elements_list = []
        for i in handcards_used_list:
            for j in i:
                handcards_used_elements_list.append(j)
        handcards_unused_list = handcards_play_rule(handcards)[1]  # 手牌单独组队后剩余的手牌
        ...
        # step05 函数返回boardcards、手牌中剩余的元素、手牌中出去的元素（破冰时用，这里可以注释掉）
        # step05-01
        boardcards = boardcards_row_list + boardcards_group_list + handcards_used_list
        # handcards_used_list = handcards - handcards_unused_list  # 列表之间不能相减
        if handcards_used_list is not []:
