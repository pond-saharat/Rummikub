# This code is an exploration of icebreaking to achieve the following(Not used as the usage code of this object)
# exploring condition: If handcards and boardcards are used at the same time, /
# check whether the sum of handcards in the deck composed of handcards and boardscards is greater than 30 to determine whether the ice breaking is successful
...
class breakIce:
    # def __init__(self, veritfy_of_break_ice, handcards, boardcards):
    #     self.veritfy_of_break_ice = veritfy_of_break_ice
    #     self.handcards = handcards
    #     self.boardcards = boardcards
    def break_the_ice(self, verify_of_break_ice, handcards, boardcards):
        if verify_of_break_ice is True:
            return verify_of_break_ice
        else:
            # Determine whether the ice can be broken under the current conditions
            if boardcards is []:
                def handcards_play_rule(self, handcards):  # handcards from grid
                    ...
                    def second_hand_list(list):
                        ...
                        for i in list:
                            if i not in first_hand_check_list and i[1] not in first_hand_check_list_value and i is not [
                                'Joker', 0]:
                    ...
                    second_hand_yellow_list = second_hand_list(
                        yellow_hand_list)  # The extra yellow card from the first round
                    second_hand_green_list = second_hand_list(
                        green_hand_list)  # The extra green card from the first round
                    ...
                    handcards_group = [value for value in result_dict.values() if
                                       len(value) > 2]  # This is a three-dimensional list
                    while True:
                    ...
                    # step-7 Determine whether the ice is broken
                    break_ice_value = 0
                    ...
                    if break_ice_value >= 30:
                        ...
                        return  # handcards.append([NewCard])


            # When there are cards on the table
            else:
                def handcards_play_rule(self, handcards):  # handcards from grid
                    handcards = [[card.colour.lower(), card.number] for card in handcards]
                    for card in handcards:
                ...
                def cs_regular_play(self, handcards, boardcards):
                    ...
                    handcards_used_list = self.handcards_play_rule(handcards)[
                        0]  # new groups formed when the hands are paired individually
                    handcards_used_elements_list = []
                    for i in handcards_used_list:
                        for j in i:
                            handcards_used_elements_list.append(j)
                    handcards_unused_list = self.handcards_play_rule(handcards)[
                        1]  # The handcards left after a single team
                    ...
                    for i in boardcards:
                        if ['Joker', 0] in i:
                            boardcards_Joker_list.append(
                                i)  # The board deck containing Joker cards is stored in this list, left behind, and added directly to the final result
                            boardcards.remove(
                                i)  # Remove the group containing Joker from the desktop deck to be processed
                        ...
                        while True:
                            if i[0][1] - 2 > 0:
                                if [i[0][0], i[0][1] - 2] in handcards_unused_list:
                                    ...
                                    handcards_unused_list.remove(
                                        [i[0][0], i[1][1] - 2])  # Remove the missing element from the handcards
                                    handcards_used_elements_list.append(
                                        [i[0][0], i[1][1] - 2])  # save the handcards that have been used
                                else:
                                    break
                            else:
                                break
                        ...
                        while True:
                            if i[-1][1] + 2 <= 15:
                                if [i[-1][0], i[-1][1] + 2] in handcards_unused_list:
                                    ...
                                    handcards_unused_list.remove(
                                        [i[0][0], i[-1][1]])  # Remove the missing element from the handcards
                                    handcards_used_elements_list.append(
                                        [i[0][0], i[-1][1]])  # save the handcards that have been used
                                else:
                                    break
                            else:
                                break
