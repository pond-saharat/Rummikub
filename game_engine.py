...
class GameEngine:
    def __init__(self, game_ui) -> None:
        ...
        # List of players
        self.players = [
            player.HumanPlayer(f"Human_player_{_}") for _ in range(self.game_ui.num_of_humans)
        ] + [player.AIPlayer(f"AI_player_{_}") for _ in range(self.game_ui.num_of_ais)]
    ...
    def endgame_score_calculation(self):
        if self.game_round < MAX_ROUND and len(self.deck.deck) > 0:
            ...
            self.winning_score = self.winners[0].score
            # self.winning_score = max([player.score for player in self.winners])
            # self.winners = score_to_players[self.winning_score]
            print(f"winners: {self.winners}")
            print(f"winning score: {self.winning_score}")
            print(f"score to players: {score_to_players}")
